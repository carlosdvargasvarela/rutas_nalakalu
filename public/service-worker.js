...
self.addEventListener("fetch", (event) => {
  ...
  // 🔥 NO INTERCEPTAR:
  // - /assets/channels/* (importmap dinámico)
  // - /cable (ActionCable WebSocket)
  // - Assets fingerprinted con hash (ya tienen cache HTTP)
  if (
    url.pathname.startsWith("/assets/channels/") ||
    url.pathname.startsWith("/cable") ||
    /\-[a-f0-9]{32,64}\.(js|css)$/.test(url.pathname) // fingerprinted assets
  ) {
  ...
  if (request.method === "GET") {
    // Assets estáticos: Cache First (solo imágenes, fuentes, iconos)
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("pending-positions")) {
        const store = db.createObjectStore("pending-positions", {
          keyPath: "id",
          autoIncrement: true,
        });
        store.createIndex("timestamp", "timestamp", { unique: false });
    ...
    for (let i = 0; i < planPositions.length; i += batchSize) {
      ...
      try {
        ...
        const response = await fetch(
          `/driver/delivery_plans/${planId}/update_position_batch`,
          {
            method: "PATCH",
            headers: {
        ...
        if (response.ok) {
          ...
          console.warn(
            `[SW] Batch sync failed for plan ${planId}:`,
            response.status,
          );
        ...
        console.error(
          `[SW] Failed to sync positions for plan ${planId}:`,
          error,
        );
        for (const pos of batch) {
          await incrementPositionRetries(pos.id);
...
function isStaticAsset(pathname) {
  // 🔥 SOLO cachear imágenes, fuentes, iconos
  // NO JS ni CSS (ya tienen fingerprint y cache HTTP)
